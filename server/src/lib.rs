use {
    rocket::{fs::NamedFile, get, routes, State}, serde::{Deserialize, Serialize}, server_api::{external::{futures, tokio::{
        fs::{self, read_dir, try_exists, File},
        io::AsyncReadExt,
    }, toml, types::{api::{APIError, APIResult, CompressedEvent}, available_plugins::AvailablePlugins, external::{chrono::{DateTime, Duration, Utc}, serde_json}, timing::{TimeRange, Timing}}}, plugin::{PluginData, PluginTrait}}, std::{collections::HashMap, path::{Path, PathBuf}, sync::Arc}
};
...
impl PluginTrait for Plugin {
    ...
    where
        Self: Sized,
    {
        AvailablePlugins::timeline_plugin_usage
    }
        ...
        for statistic in statistics {
            for (app, duration) in statistic.usage_statistic {
                ...
                resulting_events.push(CompressedEvent {
                    time: Timing::Range(statistic.timing.clone()),
                    title: app_name.clone(),
                    data: serde_json::to_value(AppEvent {
                        app: app_name,
